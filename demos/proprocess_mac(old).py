...
def pos_customers(mac):
    ...
    customer_macs = allMac1
    return customer_macs, MacOfFile_room1
...
def customerMacWithFace(mac1):
    MacToFaceNames = defaultdict(set)
    customer_macs, macOfFileRoom1 = pos_customers(mac1)
    ...
    for customer in MacToFaceNames.keys():
        if len(MacToFaceNames[customer]) < count_frame*CUSTOMER_THRESHOLD:
            MacToFaceNames.pop(customer)
        if len(MacToFaceNames[customer]) > count_frame*STATIONARY_THRESHOLD:
            MacToFaceNames.pop(customer)
    return MacToFaceNames


def macToFacePath(mac1, face1):
    rep = defaultdict(set)
    customer_PhotoName = customerMacWithFace(mac1)
...
def MacToFace(mac1, face1):
    global macToPic
    global picToRep
    macToPic = macToFacePath(mac1, face1)
